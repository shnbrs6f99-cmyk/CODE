...
class ExcelReportGenerator:
    ...
    def generate(
        ...
        wb.properties.created = datetime.now(UTC)
        wb.save(output)
        return output
    ...
    def _summary(
        ...
        rows = [
            ...
            (
                "Statement period",
                f"{result.statement.period_start or '-'} to {result.statement.period_end or '-'}",
            ),
        ...
        for row in range(8, 13):
            ws.cell(row, 2).number_format = self.MONEY
        ws.column_dimensions["A"].width = 28
        ws.column_dimensions["B"].width = 38
        self._print_setup(ws, "A1:F14")

    def _transactions(self, ws: Worksheet, result: CalculationResult) -> None:
    ...
    def _allocations(self, ws: Worksheet, result: CalculationResult) -> None:
        headers = [
            "Charge Voucher",
            "Charge Date",
            "Payment Voucher",
    ...
    def _interest(self, ws: Worksheet, result: CalculationResult) -> None:
        headers = [
            "Voucher",
            "Type",
            "Transaction Date",
        ...
        for idx, line in enumerate(result.interest_lines, start=2):
            values = [
                ...
                line.due_date,
                line.settled_date,
                float(line.principal),
                line.days_overdue,
                float(line.annual_rate) / 100,
                float(line.interest),
            ]
            for col, value in enumerate(values, start=1):
                ws.cell(idx, col, value).border = self.THIN
    ...
    def _table_header(self, ws: Worksheet, headers: list[str]) -> None:
    ...
    def _finish_table(self, ws: Worksheet, widths: list[int]) -> None:
    ...
    @staticmethod
    def _print_setup(ws: Worksheet, area: str) -> None:
        ws.print_area = area
