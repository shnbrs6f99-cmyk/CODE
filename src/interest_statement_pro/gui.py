...
class MainWindow(ctk.CTk):  # type: ignore[misc]
    ...
    def _sidebar(self) -> None:
        ...
        ctk.CTkLabel(
            side,
            text="Interest Statement\nGenerator Pro",
            font=ctk.CTkFont(size=22, weight="bold"),
            justify="left",
        ).pack(padx=22, pady=(28, 28), anchor="w")
        ctk.CTkButton(side, text="Add PDF Files", command=self._select_files, height=40).pack(
            fill="x", padx=18, pady=6
        )
        ctk.CTkButton(side, text="Add Folder", command=self._select_folder, height=40).pack(
            fill="x", padx=18, pady=6
        )
        ctk.CTkButton(
            side,
        ...
        ).pack(padx=20, pady=(28, 8), anchor="w")
        ...
        ctk.CTkLabel(
            side,
            text="Python 3.12 • Local processing",
            text_color="gray60",
        ).pack(side="bottom", pady=18)

    def _field(self, parent: Any, label: str, default: str) -> Any:
    ...
    def _workspace(self) -> None:
        ...
        ctk.CTkLabel(
            body,
            text="Select Tally Prime ledger PDFs and generate reconciled Excel interest statements.",
            text_color="gray55",
    ...
    def _select_files(self) -> None:
        ...
        self._add([Path(value) for value in values])
    ...
    def _add(self, paths: list[Path]) -> None:
        ...
        self.files.extend(path for path in paths if path not in existing)
        self._render_queue()
    ...
    def _render_queue(self) -> None:
        ...
        content = "\n".join(
            f"{index:03d}  {path}" for index, path in enumerate(self.files, 1)
        )
        self.queue.insert("end", content or "No PDFs selected.")
    ...
    def _rules(self) -> InterestRules:
        try:
            ...
            raise ValueError(
                "Rate and credit period must be valid non-negative numbers"
            ) from exc
    ...
    def _progress(self, done: int, total: int, outcome: ProcessingOutcome) -> None:
        ...
        label = f"{done}/{total}: {state} — {outcome.source.name}"
        self.after(
            0,
            lambda: (
    ...
    def _complete(self, outcomes: list[ProcessingOutcome], output: Path) -> None:
        ...
        if successes:
            try:
                ...
                pass
        details = "\n".join(
            f"• {outcome.source.name}: {outcome.error}"
        ...
        messagebox.showinfo(
            "Batch complete",
            f"Generated: {successes}\nFailed: {failures}{suffix}",
        )
    ...
    def _load_preferences(self) -> None:
        ...
        self.credit_days.delete(0, "end")
        self.credit_days.insert(0, str(days))
        self._render_queue()
